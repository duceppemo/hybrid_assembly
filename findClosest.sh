...
[ -e "${output_folder}"/genomes.list ] && rm "${output_folder}"/genomes.list #remove if exists because were are using append
for g in $(find "${output_folder}" -type f -name "*.fna.gz"); do
    echo "$g" >> "${output_folder}"/genomes.list
done

#sketch reference genomes
mash sketch \
    -p "$cpu" \
    -l "${output_folder}"/genomes.list \
    -o "${output_folder}"/genomes.msh

#compute distance
# Reference-ID, Query-ID, Mash-distance, P-value, and Matching-hashes:
mash dist \
    -p "$cpu" \
    "${output_folder}"/genomes.msh \
    "$assembly" \
